...
if __name__ == '__main__':
    ...
    # Todo: img_path_list - imgs used for evaluateions; mask_list - IMP mask
    img_path_list = ['baby', 'woman']
    mask_list = ['woman']
    ...
    for mask_name in mask_list:
        ...
        for iter in range(1, 4):
            ...
            for img_name in img_path_list:
                ...
                img_path = 'data/denoising/set_all/' + img_name + '.png'
                ...
                for name in range(1,17):
                    ...
                    pretrained = os.path.join('./init_weight_NASDIP%d.pth') % iter
                    # init_3_img_2_mask_prune_state_16.pt
                    mask_dir = os.path.join('./3_init_LTH_IMP_clean_DIP',
                                            'init_%d_img_%s_mask_prune_state_%d.pt')\
                               %(iter,mask_name,name)

                    # Todo: apply the mask to the model
                    ...
                    def closure():
                        ...
                        # Todo: noisy targets (img_noisy_torch) used for evaluations
                    ...
                    p = get_params(args.OPT_OVER, model, net_input)

                    optimize_best_model_no_scheduler(args.OPTIMIZER, p, closure, args.LR, num_iter)

                    f = open(os.path.join(args.save_dir, 'init_%d_mask_%s_img_%s_best_PSNR_start_mask_1.txt')
                             %(iter,mask_name,img_name), 'a')
                    f.write('%s \n' % str(best_psnr_gt))
                    f.close()
