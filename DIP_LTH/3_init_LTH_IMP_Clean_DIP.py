...
if __name__ == '__main__':
    ...
    parser.add_argument('--save_dir', help='The directory used to save the trained models',
                        default='./3_init_LTH_IMP_clean_DIP',
                        type=str)
    ...
    # Todo: put the image name in this list
    img_path_list = ['baby']
    ...
    for iter in range(1,4):
        ...
        for img_name in img_path_list:
            ...
            # Todo: generate your own initial weight, and then apply here
            initalization = torch.load(os.path.join('./init_weight_NASDIP%d.pth')%iter, map_location=torch.device('cuda:' + str(args.gpu)))
            ...
            for state in range(0, args.pruning_times):
                ...
                def closure():
                    ...
                    # Todo: IMP uses clean target(img_torch); Evaluation uses noisy target(img_noisy_torch)
                ...
                f = open(os.path.join(args.save_dir, 'Init_%d_best_PSNR_img_%s_from_dense.txt')%(iter,img_name), 'a')
                f.write('%s \n' % str(best_psnr_gt))
                f.close()

                # Todo: randomly prune or not
                ...
                # Todo: save the mask for evaluation
                torch.save(current_mask, os.path.join(
                    args.save_dir, 'init_%d_img_%s_mask_prune_state_%d.pt') % (iter,img_name,state+1))
