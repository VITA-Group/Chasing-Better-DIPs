...
def args():
    ...
    # Pretrained model
    PARSER.add_argument('--save_dir', type=str, default='PGAN_Random_prune_CS')
...
if __name__ == '__main__':
    ...
    sparse_weight = torch.load('./trained_weight/random/sparse_trained.pth',
    ...
    for iter_idx, (imgs, _) in enumerate(train_loader):
        ...
        def gen(z_var):
            return model.module.netG(z_var)
        ...
        y_good, A_good = data1(m_good, x0, sigma, device)
        y_bad, A_bad = data2(device, m_bad, n_features, 2)  # data3(m_bad, x, noise_degree)
        ...
        f = open(os.path.join(args.save_dir, 'MSE_PGAN_state_%d.txt')
                 % state, 'a')
        f.write('%s \n' % str(MSE_value))
        f.close()
